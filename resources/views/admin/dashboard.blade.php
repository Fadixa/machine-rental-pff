...
/* ✅ FIX: rgba blanc → txt-light */
.sidebar-brand-sub {
  font-size:.68rem; color:var(--txt-light);
  letter-spacing:.1em; text-transform:uppercase; margin-top:.1rem;
}
/* ✅ FIX: badge déjà gold-pale — renforcé */
.sidebar-admin-badge {
  display:inline-flex; align-items:center; gap:.35rem;
  background:var(--gold-pale); border:1px solid rgba(212,175,55,.35);
  color:var(--gold-dk); font-size:.7rem; font-weight:800;
...
/* ✅ FIX: couleurs blanches → txt-mid/txt-dark */
.nav-item {
  display:flex; align-items:center; gap:.7rem;
  padding:.62rem 1.5rem; cursor:pointer;
  color:var(--txt-mid); font-size:.85rem; font-weight:600;
...
.nav-item.active {
  color:var(--gold-dk); background:rgba(212,175,55,.1);
...
.sidebar-admin-info { display:flex; align-items:center; gap:.65rem; }
.admin-avatar {
  width:34px; height:34px; border-radius:50%;
  background:var(--gold); color:var(--txt-dark);
  font-weight:800; font-size:.85rem;
  display:flex; align-items:center; justify-content:center;
  flex-shrink:0; overflow:hidden; border:2px solid rgba(212,175,55,.35);
}
.admin-avatar img { width:100%; height:100%; object-fit:cover; }
/* ✅ FIX: #fff → txt-dark */
.admin-name { font-size:.82rem; font-weight:700; color:var(--txt-dark); }
/* ✅ FIX: rgba blanc → txt-light */
.admin-role { font-size:.68rem; color:var(--txt-light); }
.btn-logout-sm {
...
/* ✅ FIX: stat-card.navy accent → gold */
...
.data-card-header {
  padding:1.1rem 1.5rem; background:var(--cream);
  display:flex; align-items:center; justify-content:space-between;
  border-bottom:1px solid var(--cream3); flex-wrap:wrap; gap:.75rem;
}
...
th {
  ...
  border-bottom:1px solid var(--cream3); white-space:nowrap; text-align:left;
}
td {
  padding:.75rem 1.1rem; color:var(--txt-mid);
  border-bottom:1px solid var(--cream3); vertical-align:middle;
}
...
/* ✅ FIX: tbl-avatar navy → gold-pale */
.tbl-avatar {
  width:30px; height:30px; border-radius:50%;
  background:var(--gold-pale); color:var(--gold-dk);
  font-weight:800; font-size:.72rem;
  display:flex; align-items:center; justify-content:center;
  flex-shrink:0; overflow:hidden; border:2px solid rgba(212,175,55,.25);
}
.tbl-avatar img { width:100%; height:100%; object-fit:cover; }
/* ✅ FIX: color navy → txt-dark */
...
.modal-box {
  background:#fff; border-radius:18px; width:540px; max-width:96vw;
  box-shadow:0 24px 80px rgba(10,16,28,.2);
  transform:translateY(20px); transition:transform .3s; overflow:hidden;
}
...
.modal-close {
  ...
  background:var(--cream3); border:none; cursor:pointer; font-size:.9rem; transition:all .2s;
}
...
/* ✅ FIX: form-label color navy → txt-dark */
...
.form-control {
  ...
  color:var(--txt-dark); outline:none; transition:border .2s;
}
...
/* ✅ FIX: confirm-title navy → txt-dark */
      ...
      <div class="data-card">
        ...
        <div class="tbl-wrap">
          <table id="tblActivity">
            <thead>
              <tr><th>#</th><th>Client</th><th>Machine</th><th>Dates</th><th>Montant</th><th>Statut</th><th>Actions</th></tr>
            </thead>
    ...
    <div class="tab-panel" id="tabUsers">
      ...
      <div class="data-card">
        ...
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>#</th><th>Utilisateur</th><th>Rôle</th><th>Ville</th><th>Téléphone</th><th>Statut</th><th>Actions</th></tr>
            </thead>
            <tbody id="tbodyUsers">
              <tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--txt-light)">Chargement…</td></tr>
            </tbody>
    ...
    <div class="tab-panel" id="tabMachines">
      ...
      <div class="data-card">
        ...
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>#</th><th>Machine</th><th>Propriétaire</th><th>Type</th><th>Prix/jour</th><th>Ville</th><th>Statut</th><th>Actions</th></tr>
            </thead>
    ...
    <div class="tab-panel" id="tabReservations">
      ...
      <div class="data-card">
        ...
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>#</th><th>Client</th><th>Machine</th><th>Dates</th><th>Durée</th><th>Montant</th><th>Statut</th><th>Actions</th></tr>
            </thead>
    ...
    <div class="tab-panel" id="tabRatings">
      ...
      <div class="data-card">
        ...
        <div class="tbl-wrap">
          <table>
            <thead>
              <tr><th>#</th><th>Machine</th><th>Client</th><th>Note</th><th>Commentaire</th><th>Date</th><th>Actions</th></tr>
            </thead>
...
/* ════════════════════════════════════════════════
   ADMIN DASHBOARD — RENTIFY V13
   JS intact — zéro modification logique
════════════════════════════════════════════════ */
...
async function loadStats() {
  try {
    ...
    if (data.pending_reservations > 0) { const b=document.getElementById('badgePending'); b.textContent=data.pending_reservations; b.style.display=''; }
    if (data.suspended_users > 0) { document.getElementById('badgeSuspended').style.display=''; }
    renderCharts();
...
function renderMainChart() {
  ...
  chartMain = new Chart(ctx, {
    ...
    data:{
      labels:months,
      datasets:[
        { label:'Réservations', data:resData, backgroundColor:'rgba(15,27,45,.7)', borderRadius:6, yAxisID:'y' },
        { label:'Revenus (MAD)', data:revData, type:'line', borderColor:'#D4AF37', backgroundColor:'rgba(212,175,55,.08)', borderWidth:2.5, pointBackgroundColor:'#D4AF37', pointRadius:4, tension:.4, fill:true, yAxisID:'y1' },
      ],
    },
    options:{
  ...
  tbody.innerHTML=page.map(u=>{
    ...
    return `<tr class="${u.is_suspended?'suspended':''}">
      ...
      <td><div style="display:flex;gap:.3rem;flex-wrap:wrap">
        ...
        ${u.is_suspended?`<button class="btn-sm btn-sm-green" onclick="toggleSuspend(${u.id},false)">✅ Activer</button>`:`<button class="btn-sm btn-sm-red" onclick="toggleSuspend(${u.id},true)">🚫 Suspendre</button>`}
...
function askDelete(type,id,label) {
  const msgs={user:{icon:'👤',title:'Supprimer l\'utilisateur ?',msg:`L'utilisateur "${label}" sera supprimé définitivement.`},machine:{icon:'🏗',title:'Supprimer la machine ?',msg:`La machine "${label}" et ses données seront supprimées.`},reservation:{icon:'📋',title:'Supprimer la réservation ?',msg:`La réservation ${label} sera supprimée.`},rating:{icon:'⭐',title:'Supprimer cet avis ?',msg:`L'${label} sera supprimé définitivement.`}};
  const cfg=msgs[type];
  document.getElementById('confirmIcon').textContent=cfg.icon; document.getElementById('confirmTitle').textContent=cfg.title; document.getElementById('confirmMsg').textContent=cfg.msg;
...
function logout(){ localStorage.removeItem('auth_token'); localStorage.removeItem('auth_user'); window.location.href='/login'; }
