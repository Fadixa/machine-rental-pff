...
.auth-left-grid {
    ...
    animation: gridDrift 14s linear infinite; pointer-events: none;
}
@keyframes gridDrift { 0%{transform:translate(0,0)} 100%{transform:translate(48px,48px)} }
.circles-wrap { position: absolute; right: -80px; top: 50%; transform: translateY(-50%); pointer-events: none; }
.ring {
...
.auth-badge {
    ...
    padding: 5px 13px; border-radius: 100px; letter-spacing: .5px; margin-bottom: 20px;
}
...
.auth-left-title { font-size: 32px; font-weight: 900; color: #fff; letter-spacing: -1px; line-height: 1.15; margin-bottom: 12px; }
...
.auth-stats { display: flex; gap: 28px; position: relative; z-index: 2; }
...
.auth-form-title { font-size: 26px; font-weight: 900; color: var(--navy); letter-spacing: -.5px; margin-bottom: 4px; }
.auth-form-sub { font-size: 13px; color: var(--text-gray); margin-bottom: 28px; }
.auth-tabs { display: flex; background: #fff; border: 1px solid #E5E7EB; border-radius: var(--radius-md); padding: 3px; margin-bottom: 22px; }
.auth-tab { flex: 1; text-align: center; padding: 8px 6px; font-size: 12px; font-weight: 600; color: var(--text-gray); border-radius: 7px; cursor: pointer; transition: all .15s; display: flex; align-items: center; justify-content: center; gap: 5px; }
...
.form-input-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--text-light); font-size: 14px; pointer-events: none; }
.form-input { width: 100%; padding: 12px 14px 12px 42px; background: #fff; border: 1.5px solid #E5E7EB; border-radius: var(--radius-md); font-size: 14px; color: var(--navy); outline: none; transition: border-color .2s, box-shadow .2s; font-family: 'Inter', sans-serif; }
.form-input:focus { border-color: var(--orange); box-shadow: 0 0 0 3px rgba(245,158,11,.12); }
.form-input.error { border-color: #EF4444; }
.show-pass { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); color: var(--text-light); cursor: pointer; font-size: 14px; background: none; border: none; transition: color .2s; }
...
.btn-submit { width: 100%; padding: 13px; background: var(--navy); color: #fff; font-size: 14px; font-weight: 800; border: none; border-radius: var(--radius-md); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: transform .15s, box-shadow .15s, background .15s; letter-spacing: .2px; margin-bottom: 16px; }
.btn-submit:hover { background: #0a1421; transform: translateY(-1px); box-shadow: 0 5px 20px rgba(0,0,0,.2); }
.btn-submit:disabled { opacity: .6; cursor: not-allowed; transform: none; }
.divider-or { display: flex; align-items: center; gap: 12px; margin-bottom: 14px; }
.divider-line { flex: 1; height: 1px; background: #E5E7EB; }
.divider-text { font-size: 11px; color: var(--text-light); font-weight: 600; white-space: nowrap; }
.error-box { background: #FEF2F2; border: 1px solid #FECACA; border-radius: var(--radius-md); padding: 10px 14px; font-size: 12px; color: #B91C1C; font-weight: 500; margin-bottom: 14px; display: flex; align-items: center; gap: 8px; }
        ...
        <div class="auth-form-wrap">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
                <button onclick="showFlash('Google login — à connecter avec votre OAuth','warning')"
                    ...
                    onmouseover="this.style.borderColor='#4285F4'" onmouseout="this.style.borderColor='#E5E7EB'">
            ...
            {{-- ✅ Demo logins — emails corrects du seeder --}}
            <div style="background:#F9FAFB;border:1px solid #F0F0F0;border-radius:var(--radius-md);padding:10px 14px;margin-bottom:14px">
                <div style="font-size:10px;font-weight:700;color:var(--text-light);letter-spacing:.8px;text-transform:uppercase;margin-bottom:8px">
                    Accès démo rapide
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
                    <button onclick="fillDemo('admin@rentify.ma','password')"
                        style="padding:8px;border:1px solid #E5E7EB;border-radius:7px;background:#fff;font-size:11px;font-weight:600;color:var(--navy);cursor:pointer;transition:all .15s"
                        onmouseover="this.style.borderColor='var(--orange)'" onmouseout="this.style.borderColor='#E5E7EB'">
                        👑 Admin
                    </button>
                    <button onclick="fillDemo('karim@rentify.ma','password')"
                        style="padding:8px;border:1px solid #E5E7EB;border-radius:7px;background:#fff;font-size:11px;font-weight:600;color:var(--navy);cursor:pointer;transition:all .15s"
                        onmouseover="this.style.borderColor='var(--orange)'" onmouseout="this.style.borderColor='#E5E7EB'">
                        🏗 Owner
                    </button>
                    <button onclick="fillDemo('fadwa@rentify.ma','password')"
                        style="padding:8px;border:1px solid #E5E7EB;border-radius:7px;background:#fff;font-size:11px;font-weight:600;color:var(--navy);cursor:pointer;transition:all .15s"
                        onmouseover="this.style.borderColor='var(--orange)'" onmouseout="this.style.borderColor='#E5E7EB'">
                        🙋 Client
                    </button>
...
/* ── Fonction centrale de redirect ── */
function redirectByRole(role) {
    const base = window.location.origin;
    if (role === 'admin')  { window.location.href = base + '/dashboard/admin';  return; }
    if (role === 'owner')  { window.location.href = base + '/dashboard/owner';  return; }
    window.location.href = base + '/dashboard/client';
}
let loginTab = 'email';
